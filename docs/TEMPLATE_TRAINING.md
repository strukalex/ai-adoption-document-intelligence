...
- Key-value fields: from `analyzeResult.keyValuePairs`, mapped to existing word boxes.
  ...
  - When several OCR keys match the same field (e.g. "Applicant Print Name" and "Spouse Print Name" both match alias "name"), the pair whose key matches the **longest** alias is chosen (e.g. "spouse name" over "name"), so "Spouse Print Name" maps to `spouse_name` and "Applicant Print Name" to `name`.
